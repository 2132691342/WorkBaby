...
// MicroCompressor 确定性折叠器：不调 LLM，只做两件事——
// 折叠最旧的「assistant(tool_calls) + 连续 tool 结果」整段，仍超预算则按轮裁剪。
//
// 两条路径都以段为单位，绝不制造孤儿 tool 或空 assistant 占位。
...
func (MicroCompressor) Compress(msgs []*llm.Message, budget int) ([]*llm.Message, CompressInfo) {
	...
	for totalTokens(out) > budget {
		i := oldestToolSegment(out)
		if i < 0 {
			break
		}
		before := totalTokens(out)
		info.Removed += collapseSegment(out, i)
		if totalTokens(out) >= before {
			break
		}
	}
...
// 返回被折叠的条数。
func collapseSegment(msgs []*llm.Message, i int) int {
	head := *msgs[i]
